...
-- Remove these unneeded functions related to unidentified items

--[[
--- Mark an item as unidentified
-- @param self Item: The item to mark
...
-- @param self Item: The item to check
-- @return boolean: True if the item is unidentified
function Item.isUnidentified(self)
    return self:getCustomAttribute("unidentified")
end

--- Identify an item, revealing its properties
-- @param self Item: The item to identify
-- @param player Player: The player identifying the item
-- @param itemType ItemType: The type of the item
-- @param weaponType number: The weapon type
-- @return boolean: True if successful
function Item.identify(self, player, itemType, weaponType)
    ...
    -- Check if item can become unique
    local canUnique = false
    for i = 1, #US_UNIQUES do
        if US_UNIQUES[i].minLevel <= self:getItemLevel() and 
           bit.band(usItemType, US_UNIQUES[i].itemType) ~= 0 then
            canUnique = true
            break
        end
    end
    ...
    if canUnique and math.random(US_CONFIG.UNIQUE_CHANCE) == 1 then
        ...
        while US_UNIQUES[unique].minLevel > self:getItemLevel() or 
              bit.band(usItemType, US_UNIQUES[unique].itemType) == 0 or 

...
function gameStoreUpdateHistory(player)
	...
	local history = {}
	local resultId = db.storeQuery("SELECT * FROM `shop_history` WHERE `account_id` = " .. player:getAccountId() .. " order by `id` DESC")
	...
	if resultId ~= false then
		repeat
			...
			if target ~= "" then
				desc = desc .. " on " .. result.getDataString(resultId, "purchase_date") .. " for " .. target .. " for " .. result.getDataInt(resultId, "price") .. " points."
			else
				desc = desc .. " on " .. result.getDataString(resultId, "purchase_date") .. " for " .. result.getDataInt(resultId, "price") .. " points."
			end
